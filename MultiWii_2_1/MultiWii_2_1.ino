...
static uint16_t serialFrequenceTime = 0;
static uint8_t updateStatusCmd[4] = {0,0,0,0};          // update status cmd
...
void annexCode() { // this code is excetuted at each loop and won't interfere with control loop if it lasts less than 650 microseconds
  ...
  if ( currentTime > calibratedAccTime ) {
    ...
    if(GPS_Enable == 0) {
        //serialCom();
        if (currentTime > serialTime && serialFrequenceTime > 0) {
            serialTime = currentTime + serialFrequenceTime;
            serialCom(1);
        } else {
            serialCom(0);
        }
