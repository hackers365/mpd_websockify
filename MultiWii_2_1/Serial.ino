...
#define MSP_SET_FREQUENCE_TIME   150   //in message
#define MSP_ADD_UPDATE_STATUS    151   //in message
...
void serialCom(uint32_t get_status) {
  uint8_t c;
...
void evaluateCommand() {
  switch(cmdMSP) {
   ...
   case MSP_DEBUG:
     ...
     break;
    case MSP_SET_FREQUENCE_TIME:
        serialFrequenceTime = read16();
        headSerialReply(0);
        break;
    case MSP_ADD_UPDATE_STATUS:
         debug[1] = 101;
         for(uint8_t i=0;i<4;i++) {
            updateStatusCmd[i] = read8();
            //debug[i] = updateStatusCmd[i];
         }
         headSerialReply(0);
     break;
